...
class RequestBuilder:
    ...
    def new_auth_headers() -> Dict[str, str]:
        ...
        return {
            ...
            "X-Api-Key": "ark-84338bf6-d05a-48e7-b3b3-0b06ab99b556-33167",
        }
    ...
    async with AsrWsClient(args.url, args.seg_duration) as client:
        try:
            async for response in client.execute(args.file):
                try:
                    pm = response.payload_msg
                    if isinstance(pm, dict) and pm.get("result",{}).get("text"):
                        final = pm["result"]["text"]
                except: pass
        except Exception as e:
            logger.error(f"ASR processing failed: {e}")
    print("ASR_RESULT:" + json.dumps(final, ensure_ascii=False))
